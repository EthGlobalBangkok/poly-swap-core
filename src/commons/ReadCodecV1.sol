...
library ReadCodecV1 {
    ...
    function decode(bytes calldata _cmd)
        internal
        pure
    ...
    function decodeRequestsV1(bytes calldata _cmd, uint256 _offset)
        internal
        pure
        returns (EVMCallRequestV1[] memory evmCallRequests, uint256 newOffset)
    {
        newOffset = _offset;
        ...
        for (uint16 i = 0; i < requestCount; i++) {
            ...
            if (resolverType == RESOLVER_TYPE_SINGLE_VIEW_EVM_CALL) {
                (EVMCallRequestV1 memory request, uint256 nextOffset) =
                    decodeEVMCallRequestV1(_cmd, newOffset, appRequestLabel);
    ...
    function decodeEVMCallRequestV1(bytes calldata _cmd, uint256 _offset, uint16 _appRequestLabel)
        internal
        pure
        returns (EVMCallRequestV1 memory request, uint256 newOffset)
    {
        newOffset = _offset;
    ...
    function decodeEVMCallComputeV1(bytes calldata _cmd, uint256 _offset)
        internal
        pure
        returns (EVMCallComputeV1 memory compute, uint256 newOffset)
    {
        newOffset = _offset;
    ...
    function encode(uint16 _appCmdLabel, EVMCallRequestV1[] memory _evmCallRequests)
        internal
        pure
    ...
    function appendEVMCallRequestV1(bytes memory _cmd, EVMCallRequestV1 memory _request)
        internal
        pure
        returns (bytes memory)
    {
    ...
    function appendEVMCallComputeV1(bytes memory _cmd, EVMCallComputeV1 memory _compute)
        internal
        pure
        returns (bytes memory)
    {
